...
**Lean verifies the complete answers for paths, two-row grids, and the $3\times3\times3$ box.** The other geometric classifications currently have ordinary mathematical proofs. A successful build of the formal archive should not be read as formal verification of every result in the manuscript.
...
The complete three-row and four-row formulas, the five-row geometric argument, the all-odd-box isoperimetric theorem, the application to boxes with a side of length two, and the general cylinder construction still have ordinary proofs. The [full proof text](/princess/proofs/) states their hypotheses and arguments. Numerical experiments elsewhere in the research archive are explicitly distinguished from proofs.
