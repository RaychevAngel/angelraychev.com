...
There are 18 Lean source modules, including the axiom-reporting `Audit.lean`. The finite certificates are checked by **kernel reduction** (`decide +kernel`), not native execution or an external solver assertion. The small graphs and all certificate data are explicit in the Lean source. The general vertex bound and the exact `(12,24)` extremal cell are now connected through the same formal score definition.
...
`Guaranteed E s t` means that every allowed initial cop has some finite `Winning` tree. Its positive interpretation against every legal history-dependent cop is proved by `DeliveryGame`. The new losing-region theorem rules out all such finite trees; a separate completeness theorem identifying this relation with every possible unbounded history-dependent messenger strategy is not yet formalized. The score, general vertex bound and exact-cell theorem use this stated finite-tree definition. The three-vertex pilot proves full coverage of that small graph space; it does not formalize the larger external degree-profile enumerations, symmetry reductions, pruning or finite tables. No full-classification theorem is claimed.
